...
namespace dae
{
	Camara::Camara(const Vector3 origin, float fovAngle, float aspectRatio)
	{
		Initialize(fovAngle, origin, aspectRatio);
	}

	void Camara::Initialize(float _fovAngle, Vector3 _origin, float _aspectRatio)
	{
	...
	void Camara::Update(const Timer* pTimer)
	{
