...
float4 PS(VS_OUTPUT input) : SV_Target
{
    ...
    float4 ambientcolor = float4(0.025f,0.025f,0.025f,1);
    float3 SampledNormal = input.normal;
    float LightIntesity = 7.0f;
    float Shininess = 25.0f;
    float PhongSpecular = 1.0f;
    ...
    Shininess *= g_GlossMap.Sample(g_TextureSampler, input.TextureUV);
    ...
    float4 phongColor = float4(1, 1, 1, 1) * PhongSpecular * pow(cosAlpha, Shininess);
